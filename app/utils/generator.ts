...
export function generateMulti<T extends object>(
  generator: (specified?: Partial<T>) => T,
) {
  ...
  return (size: number, specifyData?: (index: number) => Partial<T>): T[] => {
    ...
    return times(size).map(order =>
      needSpecifyData ? toGenerateOne(specifyData(order)) : toGenerateOne(),
    );
  };
}
